...
def _GetMSBuildGlobalProperties(spec, guid, gyp_file_name):
  ...
  if spec.get('msbuild_enable_android'):
    properties[0].append(['Keyword', 'Android'])
    properties[0].append(['MinimumVisualStudioVersion', '14.0'])
    properties[0].append(['ApplicationType', 'Android'])
    properties[0].append(['ApplicationTypeRevision', '1.0'])

  elif spec.get('msvs_enable_winrt'):
    properties[0].append(['Keyword', 'Win32Proj'])
...
def _GetMSBuildLocalProperties(spec, msbuild_toolset):
  default_config = _GetDefaultConfiguration(spec)
  ...
  if default_config.get('msbuild_cpp_stdlib'):
    msbuild_cpp_stdlib = default_config.get('msbuild_cpp_stdlib')
    properties[0].append(['UseOfStl', msbuild_cpp_stdlib])
...
def _GetMSBuildConfigurationGlobalProperties(spec, configurations, build_file):
  ...
  for (name, configuration) in sorted(configurations.iteritems()):
    ...
    property_name = {'IntermediateDirectory': 'IntDir',
                     'OutputDirectory':       'OutDir'}
                     
    for a in attributes:
        _AddConditionalProperty(properties, condition, property_name.get(a, a),
                                attributes[a])
...
def _GenerateMSBuildProject(project, options, version, generator_flags):
  ...
  if spec.get('msvs_enable_winphone'):
   ...
   content += _GetMSBuildLocalProperties(spec, project.msbuild_toolset)
